...
export const Header = ({ title, subtitle }: HeaderProps) => {
  const { plantType, setPlantType } = usePlant();
  ...
  return (
    <header className="h-16 bg-card border-b border-border flex items-center justify-between px-8">
      <div className="flex items-center gap-6">
        ...
        {/* Plant Type Toggle */}
        <div className="flex items-center bg-secondary rounded-lg p-1" role="tablist" aria-label="Plant type">
          <button
            type="button"
            role="tab"
            aria-selected={plantType === 'fusion'}
            onClick={() => setPlantType('fusion')}
            className={cn(
          ...
          <button
            type="button"
            role="tab"
            aria-selected={plantType === 'pwr'}
            onClick={() => setPlantType('pwr')}
            className={cn(
              'flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-all',
              plantType === 'pwr'
                ? 'bg-primary text-primary-foreground shadow-sm'
